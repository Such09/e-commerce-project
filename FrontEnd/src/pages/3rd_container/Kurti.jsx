...
const Kurti = () => {
  ...
  const data = async () => {
    try {
      const response = await axios.get(`https://e-commerce-project-2-72zj.onrender.com/ecommerce/v1/find_product?items=kurti`)
  ...
  return (
    <div className='min-h-screen w-full py-7 bg-gray-200 flex flex-col gap-5  '>
      <div className='h-full w-full px-6 flex flex-wrap items-center gap-10'>
        {
          kurtiData.map((data) => (
            <Link key={data._id} to={`/app/kurti/kurti_card/${data._id}`}>
