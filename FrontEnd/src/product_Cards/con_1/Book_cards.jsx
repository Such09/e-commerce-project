...
const Book_card = () => {
    ...
    const fetchProduct = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/ecommerce/v1/book/${id}`)
                ...
                <div key={getData._id} className='h-full w-full flex gap-10'>
                    ...
                    <div className='h-full w-1/2 px-4 flex flex-col gap-5'>
                        <div className='w-full p-3 flex flex-col gap-2 border-2 border-gray-400/35 rounded'>
                            <h1 className='text-lg font-medium'> {getData.name} </h1>
                            <h1 className='font-medium'>Author:{getData.author} </h1>
                            ...
                            <div className='flex gap-3 items-center'>
                                ...
                                <h1 className='font-medium text-green-800'> {getData.tag} </h1>
                            </div>

                             <h1> {getData.offerTag} </h1>
                        </div>
