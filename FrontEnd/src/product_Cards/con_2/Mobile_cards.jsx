...
const Mobile_card = () => {
    ...
    const fetchProduct = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/ecommerce/v1/mobile/${id}`)
                ...
                <div key={getData._id} className='h-full w-full flex gap-10'>
                    ...
                    <div className='h-full w-1/2 px-4 flex flex-col gap-5'>
                        <div className='w-full p-3 flex flex-col gap-2 border-2 border-gray-400/35 rounded'>
                            ...
                            <h1 className='font-medium'> {getData.model} </h1>

                            <p className='font-medium text-gray-500'>Camera: {getData.camera} </p>
